// Package scheduler decides which parameter vectors to run next, given the
// current set of observations. The phase-1 Static scheduler does an LHS batch
// followed by adaptive neighbor rounds; the same Scheduler interface will later
// be implemented by a Bayesian-optimization driver with no change to callers.
package scheduler
...
// State carries the information a Scheduler uses to choose the next batch.
type State struct {
...
type Scheduler interface {
	Next(ctx context.Context, st State) (vectors []experiment.ParamVector, phase string, err error)
}

// Static drives a fixed two-phase exploration: an initial LHS sample batch,
// then a number of adaptive neighbor rounds around the best observations.
type Static struct {
...
func (s *Static) Next(ctx context.Context, st State) ([]experiment.ParamVector, string, error) {
	if err := ctx.Err(); err != nil {
		return nil, "", err
	}
	if s.round == 0 {
		s.round++
		vectors, err := sampler.LHS{Seed: s.Seed}.Sample(st.Enc, s.LHSn)
		if err != nil {

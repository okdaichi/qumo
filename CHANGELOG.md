...
- **Standalone browser logging library (`log/`):** New self-contained, framework-agnostic logging package (`@okdaichi/log`, publishable to jsr) living in a top-level `log/` directory with its own `deno.json`. Tagged (`createLogger(tag)`), level-filtered (trace/debug/info/warn/error, runtime `setLevel` global or per-tag), structured (`log.info(msg, { fields })` — values kept as-is so suppressed logs pay nothing to build), with consecutive message-only dedup into a single `×N` entry, `throttle()` rate-limiting for noisy structured logs, and `counter()` aggregation for genuinely high-frequency (per-frame) paths where `mark()` is O(1) and a shared 1s timer flushes one summary line. A 1024-entry preallocated ring buffer retains recent entries for `exportLogs()` (text or ndjson, with `Error` fields serialized) for bug reports; pluggable sinks (`addSink`/`removeSink`) plus the built-in console sink; `onLogs()`/`onLevelChange()` subscribe hooks for a future debug UI. Zero runtime dependencies, no `import.meta.env` coupling — production quietness is a runtime level concern (`setLevel("warn")`), and the aggregator timer unrefs in Deno so it can't hang tests/SSR. `deno test` (11 tests), `deno lint`, `deno fmt` all clean.

...
- **Automation-friendly relay-chain benchmark suite (`internal/relay`, `scripts`, #284):** The relay-chain benchmarks now emit machine-readable JSONL results (`BENCH_RESULTS_DIR`), the fan-out sweep honors a `FANOUT_KS` env override (so CI can trim the K range), and a new `TestRelayChain_ReconnectStorm` characterizes goroutine/heap behavior under subscriber churn (runs in the bench workflows via `RUN_STORM=1`, skipped in the per-PR CI gate). A zero-dependency Deno/TS report generator (`scripts/relay_bench_report.ts`) turns the JSONL into CSV + SVG plots. Two workflows run it: `bench-relay-pr.yml` (label-gated ~10-min smoke) and `bench-relay.yml` (nightly full sweep + soak, plus an on-demand `workflow_dispatch` with a 30m/1h/3h/6h soak-duration choice). The per-PR CI integration gate now skips the heavy `TestRelayChain_*` durability tests (`-skip='RelayChain'`); they run in the relay-bench workflows instead, which are their intended home.

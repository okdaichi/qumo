...
- **Centralized frontend logging (`playground`):** Direct `console.*` calls are replaced by a single tagged, level-filtered logger (`src/log.ts`) — the 22 prior call sites across the publish/subscribe boards, `media.ts`, and `cert.ts` now go through `createLogger(tag)`. Levels (trace/debug/info/warn/error) are checked with one numeric compare before any argument is touched, so a suppressed log is near-free; the per-frame encode/decode loops stay allocation-conscious and gain no logging unless they opt into the `counter()`/`throttle()` high-frequency-safe helpers (modeled on `stats.ts`'s `createStatsTicker`). Message-only repeats dedup into a single `×N` line; noisy structured logs can be rate-limited; a 1024-entry ring buffer retains recent logs for `exportLogs()` (a bug-report transcript that serializes `Error` fields), reachable in dev via `window.qumoLogs` (`setLevel`/`getLevel`/`exportLogs`). `trace`/`debug` bodies are gated behind `import.meta.env.DEV` so Vite strips them from the production bundle; `info`/`warn`/`error` survive as real prod diagnostics. A `git grep` CI step in `ci.yml` now forbids new `console.*` outside `src/log.ts`. A SolidJS `<DebugPanel/>` live viewer is a planned follow-up; `onLogs()`/`onLevelChange()` already expose the subscribe hooks for it.

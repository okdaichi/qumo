...
- **Media-domain logging library (`log/`, `@qumo/log`):** New self-contained logging library for real-time media apps, living in a top-level `log/` directory (own `deno.json`, publishable to jsr as `@qumo/log`). Media-first but stack-agnostic — no dependency on WebCodecs/WebTransport/MSE/WebRTC/`@qumo/moq`; the caller feeds it numbers and it produces the rates a media pipeline needs. The generic engine (`createLogger`, levels trace..error, runtime `setLevel` global or per-tag, structured fields kept as-is so suppressed logs pay nothing to build, consecutive message-only dedup into a `×N` entry, `throttle()` rate-limiting, `counter()` aggregation, 1024-entry preallocated ring buffer + `exportLogs()` text/ndjson with `Error` fields serialized, pluggable sinks, `onLogs()`/`onLevelChange()` hooks) is public, and a media layer (`media.ts`) adds: curated `MediaTags` (audio/video/encoder/decoder/renderer/capture/transport/network/moq/quic/webtransport/jitter/catalog), typed meters on a shared 1s pulse — `meter.fps()` → `N.n fps (total M)`, `meter.bitrate()` → auto-scaled `kbps/Mbps/Gbps`, `meter.rate()` → `N.n/s`, `meter.gauge()` → `avg/min/max` with a unit suffix (decode-queue depth, RTT, jitter) — where `mark()`/`sample()` are O(1) and allocation-free (safe in per-frame loops); media-timestamp (PTS) stamping so every entry aligns to the stream timeline (`createMediaLogger(tag, { clock })` stamps `mts` on every emit; `frame()` uses a `pts` field directly); and `formatMediaTime`/`formatBitrate` helpers. Zero runtime deps; no `import.meta.env` coupling (production quietness is `setLevel("warn")`); the single flush timer unrefs in Deno so it can't hang tests/SSR. `deno test` (20 tests), `deno lint`, `deno fmt` all clean.

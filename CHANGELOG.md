...
- **`internal/relay` GC tuning for high-fan-out capacity (`RELAY_GOGC`, default 200).** A fan-out relay holds a large, *stable* live set — one QUIC connection per subscriber, whose ~9 goroutines' stacks dominate RSS (measured: Go heap in-use ~200MB while RSS ~1.4GB at ~14K sessions; the gap is off-heap goroutine stacks). Every GC cycle re-scans all those stacks, so at the default `GOGC=100` the GC-scan CPU grows with connection count and becomes the scaling ceiling (measured on 8 cores: default holds ~13–15K sessions; the collapse is GC, not memory exhaustion — RSS/session is flat ~127KB). `cmd.go` now calls `debug.SetGCPercent` at startup: if `GOGC` is set the runtime owns it (we never stomp it); else `RELAY_GOGC` (a valid positive int) applies; else a modest relay default of **200**. Because the live set is legitimate and stable, collecting it less often costs only some peak RSS headroom while cutting GC CPU — raising `RELAY_GOGC` to 600–1600 reached **~18–20K concurrent subscriber sessions** on an 8-core host (`BenchmarkRelay_ConnectionCarry`, slow-ramp), roughly doubling the default ceiling. **`GOMEMLIMIT` is deliberately not used** — capping memory for this large-stable-live-set forces constant GC into a death-spiral (measured: GOMEMLIMIT configs collapsed to ~15–18K while `GOGC=high` held 20K). Documented in `relay-config.example.env`; the decision logic (`gcPercent`) is unit-tested for env precedence.

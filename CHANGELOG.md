...
- **RTSP interop test no longer flaky on the GOP=60 case (`internal/integration`):** `TestRTSPInterop_Matrix/gop60_720p30` intermittently failed the PTS-regression gate on CI when its 2-second capture window straddled a GOP boundary — the gate's CTS window (1 s, shared with the GOP≤30 cases) was smaller than the legitimate hierarchical B-frame reorder depth at GOP=60 @ 30 fps (CI observed a 1.97 s regression, ≈ one full GOP). The `gop60_720p30` case now uses a 2.5 s `MaxCTSWindowUS` (theoretical max reorder is one GOP = 2 s, plus margin for jitter). No product change; the RTSP ingest PTS math was correct (the RTMP equivalent under the same collector/gate passes). The flake was not reproducible off CI.
...
- **`mage cert` prefers mkcert for local dev (`magefiles`, `playground`, `docker`):** `mage cert` now signs the localhost WebTransport cert with [mkcert](https://github.com/FiloSottile/mkcert) when it's on PATH, producing a long-lived cert that chains to a trusted local root CA — so the browser trusts it directly and no `VITE_CERT_HASH` pinning, no 14-day re-run, and no Vite restart are needed. A stale `VITE_CERT_HASH` from a prior self-signed run is cleared from `playground/.env` so it can't pin the wrong cert. A new `CERT_HOSTS` env var (comma-separated, mkcert path only) appends extra SANs beyond the default `localhost`/`127.0.0.1`/`::1`, so the cert also validates when the demo is reached from another device on the LAN (e.g. `CERT_HOSTS=192.168.1.10,desktop.local mage cert`). When mkcert is absent (CI, headless, air-gapped), `mage cert` falls back to the previous 14-day self-signed ECDSA cert and writes `VITE_CERT_HASH` as before, so those workflows are unchanged. The browser consumer (`playground/src/cert.ts`, `config.ts`) already treated the hash as optional, so no client change was needed. Closes #196.
